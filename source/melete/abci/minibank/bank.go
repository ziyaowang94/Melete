...
func (app *Application) PreExecutionC(block *types.Block) *types.ABCIPreExecutionResponseC {
	fmt.Printf("%v Cshard_preexecute\n", time.Now())
	memoryDB := NewMemoryDB(app.db)
	...
	fmt.Printf("%v Cshard_preexecuteend\n", time.Now())
	return &types.ABCIPreExecutionResponseC{
		Code:            types.CodeTypeOK,
...
func (app *Application) Execution(CrossShardBlocks []*types.Block, commitBitVector []*utils.BitVector, InnerShardBlocks []*types.Block) *types.ABCIExecutionResponse {
	...
	executionC := func(block_sequence int, block *types.Block) {
		defer wg.Done()
		...
		for i, tx := range block.CrossShardTxs {
			...
			if err != nil {
				...
				abciResp.Log = "C PreExecution Abort"
	...
	for block_sequence, block := range CrossShardBlocks {
		go executionC(block_sequence, block)
	}
	wg.Wait()
