	...
	if im.isI {
		...
		go func() {
			...
			if rate > 0 {
				...
				log.Printf("I-shard start mempool，Broadcast interval：%d/%v", rate, dur)
				for len(im.txPool) > 0 {
					startTime := time.Now()
					dst := batch
					...
					for _, tx := range im.txPool[:dst] {
						...
						ctx := types.Tx(TransferBytes(tx))
						im.mempool.AddTx(&ctx)
						if im.p2pConn != nil {
							im.p2pConn.SendToShard(im.myChain, p2p.ChannelIDMempool, []byte(ctx), definition.TxTransfer)
						}
					}
				...
				log.Println("=======================================")
				log.Println("        transaction broadcast complete                   ")
				log.Println("=======================================")
			}
		}()
	} else {
		...
		if rate1 > 0 {
			...
			log.Printf("C-shard start mempool，Broadcast interval：%d/%v", rate1, dur1)
			go func() {
				for len(im.txPool) > 0 {
					startTime := time.Now()
					dst := batch
		...
		if rate2 > 0 {
			...
			log.Printf("C-shard start mempool，Broadcast interval：%d/%v", rate2, dur2)
			go func() {
				...
				log.Println("==============================================")
				log.Println(" cross-shard transaction broadcast complete               ")
				log.Println("==============================================")
			}()
		}
	}
