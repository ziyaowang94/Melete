...
func (im *Importor) StartMempool() {
	if im.isI {
		...
		if rate1 > 0 {
			...
			log.Printf("B-shard start mempool，Broadcast interval：%d/%v", rate1, dur1)
			go func() {
				for len(im.txPool) > 0 {
					startTime := time.Now()
					dst := batch
		...
		if rate2 > 0 {
			...
			log.Printf("B-shard start mempool，Broadcast interval：%d/%v", rate2, dur2)
			go func() {
				...
				log.Println("=======================================")
				log.Println(" cross-shardtransaction broadcast complete               ")
				log.Println("=======================================")
			}()
		}
	}
