...
func (cs *ConsensusState) IsCShard() bool { return cs.heightDatas.IsCShard() }
...
func (cs *ConsensusState) handleStateTransition() error {
	...
	case RoundStepCrossShardAccept: // RoundStepCrossShardProposal
		...
		} else if cs.IsCShard() && cs.heightDatas.FinishCrossShardProposal() {
	...
	case RoundStepCrossShardCommit:
		if cs.IsIShard() && cs.heightDatas.FinishCrossShardCommit_I() {
			...
			cs.doCSC_I()
		} else if cs.IsCShard() && cs.heightDatas.FinishCrossShardCommit_B() {
			cs.WriteLogger("Finish CrossShardCommit", false, false)
			cs.doCSC_C()
		} else {
			return nil
...
func (cs *ConsensusState) doPropose() {
	...
	if cs.IsCShard() && cs.calculateProposer() != cs.signerIndex {

		resp := cs.abci.PreExecutionC(proposalBlock)
		if resp.Code != types.CodeTypeOK || !utils.CompareBytesList(resp.CrossShardDatas, proposalBlock.CrossShardDatas) {

			panic("C shard pre-execution error")
		}
	}
...
func (cs *ConsensusState) doPrecommit() {
	var shardType uint32 = constypes.ShardTypeC
	if cs.IsIShard() {
...
func (cs *ConsensusState) doCSC_C() {
	c_neighbours := make([]string, 0, len(cs.heightDatas.ShardInfo.Related2LevelShards)+1)
	c_neighbours = append(c_neighbours, cs.heightDatas.MyChainID)
	...
	for shard := range cs.heightDatas.ShardInfo.Related2LevelShards {
		c_neighbours = append(c_neighbours, shard)
	}
	cs.sortShards(c_neighbours)
	for shard := range cs.heightDatas.ShardInfo.RelatedShards {
		i_neighbours = append(i_neighbours, shard)
	}
	resp, commitStatus := cs.executionAllBlocks(c_neighbours, i_neighbours)

	cs.ChainedHash = make([][]byte, 0, len(i_neighbours)+len(c_neighbours)-2)
	cs.RelatedCommitStatus = make([][]byte, 0, len(c_neighbours)-1)
	for i, cshard := range c_neighbours {
		if cshard == cs.heightDatas.MyChainID {
			cs.CommitStatus = commitStatus[i]
			block := cs.heightDatas.CrossShardBlocks[cshard]
			cs.BlockHash = block.Hash()
			cs.LastBlockTime = block.Time
		} else {
			cs.RelatedCommitStatus = append(cs.RelatedCommitStatus, commitStatus[i])
			cs.ChainedHash = append(cs.ChainedHash, cs.heightDatas.CrossShardBlocks[cshard].Hash())
		}
	}
	...
	for _, shard := range c_neighbours {
...
func (cs *ConsensusState) doCSC_I() {
	c_neighbours, i_neighbours := make([]string, 0, len(cs.heightDatas.ShardInfo.RelatedShards)), []string{cs.heightDatas.MyChainID}
	for shard := range cs.heightDatas.ShardInfo.RelatedShards {
		c_neighbours = append(c_neighbours, shard)
	}
	cs.sortShards(c_neighbours)
	...
	cs.ChainedHash = make([][]byte, 0, len(c_neighbours))
	cs.RelatedCommitStatus = make([][]byte, 0, len(c_neighbours))
	for i, cshard := range c_neighbours {
		cs.RelatedCommitStatus = append(cs.RelatedCommitStatus, commitStatus[i])
		cs.ChainedHash = append(cs.ChainedHash, cs.heightDatas.CrossShardBlocks[cshard].Hash())
	}
...
func (cs *ConsensusState) executionAllBlocks(c_neighbours, i_neighbours []string) (*types.ABCIExecutionResponse, [][]byte) {
	c_blocks := make([]*types.Block, len(c_neighbours))
	i_blocks := make([]*types.Block, len(i_neighbours))
	commitStatuses := make([]*utils.BitVector, len(c_neighbours))
	commitStatusBzList := make([][]byte, len(c_neighbours))
	for i, shard := range c_neighbours {
	...
	return cs.abci.Execution(c_blocks, commitStatuses, i_blocks), commitStatusBzList
}
...
func (cs *ConsensusState) createBlockAndBroadcast() {
	...
	if cs.IsCShard() {
		resp := cs.abci.PreExecutionC(block)
