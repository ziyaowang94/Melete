...
const (
	...
	defaultProtocal         = ProtocolMelete
...
func GenerateConfigFiles(shard_config_path string, store_dir string) {
	...
	for _, si := range shardConfig.Shards {
		...
		shardInfo.Related2LevelShards = make(map[string]bool)
		for _, shard := range si.RelatedShards {
			shardInfo.RelatedShards[shard] = true
			for _, shard2 := range PeerRelatedMap[shard] {
				shardInfo.Related2LevelShards[shard2] = true
			}
		}
		delete(shardInfo.Related2LevelShards, si.ChainID)
		for i := 0; i < int(si.PeerNum); i++ {
