...
import (
	"context"
	"emulator/rapid/abci/minibank"
	"emulator/rapid/consensus/tendermint"
	"emulator/rapid/definition"
	"emulator/rapid/mempool"
...
func InitNode(rootDir string, startTimeStr string, waitTime string) {
	...
	fmt.Println(time.Until(time_to_start(startTimeStr)))
...
func createConsensus(cfg *Config, si *shardinfo.ShardInfo, s *signer.Signer, mmp, cmmp definition.MempoolConn,
	...
	default:
		panic("An undefined consensus interface")
	}
}
...
func time_to_start(startTimeStr string) time.Time {
	...
	if err != nil {
		panic(fmt.Sprintln("Startup time format error :", err))
